...
async function run() {
  ...
  let testDigits: any = [];
  try {
    trainLabels = await mnist.getTrainLabels();
    trainDigits = await mnist.getTrainImages();

    testLabels = await mnist.getTestLabels();
    testDigits = await mnist.getTestImages();
  } catch (error) {
    throw new Error(error);
  }

  const trainDigitsTensor = tf.tensor4d(trainDigits.map(mnist.normalize), [
    Math.floor(trainDigits.length / (28 * 28)),
    28,
    28,
    1,
  ]);
  ...
  const testDigitsTensor = tf.tensor4d(testDigits.map(mnist.normalize), [
    Math.floor(testDigits.length / (28 * 28)),
    28,
    28,
    1,
  ]);
  console.log('>>>>>>>>>>>>', testDigits, testDigitsTensor)
